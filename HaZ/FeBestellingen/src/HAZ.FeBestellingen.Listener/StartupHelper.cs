...
namespace HAZ.FeBestellingen.Listener
{
    public class StartupHelper
    {
        ...
        public static DbContextOptions<BestellingenContext> GetDbContext(IConfigurationRoot configuration)
        {
        ...
        public static void WaitForMysql(ILogger<Program> logger, IConfigurationRoot configuration)
        {
            ...
            while (!online)
            {
                try
                {
                    ...
                    using (var bestelingRepo = new BestellingRepository(new BestellingenContext(GetDbContext(configuration))))
                    {
